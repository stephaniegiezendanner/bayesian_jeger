...
### beta distribution:
dfn$y<-dfn$rWC/100
dfn$y[which(dfn$y==0)]<-0.01
dfn$y[which(dfn$y==1)]<-0.99
hist(dfn$y)

model<-bf(y ~ MICF_mean  +(1|psy) )
...
plogis (-1.46) # 0.1884673 → the mean response decreases toward 18%
...
predict(mod_b,newdata=newdata, re_formula =NULL,summary=T, robust=T)
...
# R2 0.5704499 0.01219012 0.5449191 0.5930418
...
p<-ggplot(results, aes(x = predicted, y = observed)) +
  ...
  theme_minimal()
print(p)
ggsave(filename = file.path("Figures/Bayesian","LMER_beta_predictions_psy.tiff" ),
...
server <- function(input, output) {
  observeEvent(input$predict, {
    ...
    output$text <- renderText({  
      paste("Percentage of draws lying within different intervals in WC ratings that stakeholders find acceptable when two experts independently assess the same patient")
      })
