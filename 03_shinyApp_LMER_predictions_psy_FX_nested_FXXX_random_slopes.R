...
mod_priors <- c(  prior(normal(95, 30), class = Intercept), # intercept
  # slopes
...
mod_regression_rs = brms::brm(
  rWC ~ MICF_mean  +(MICF_mean|psy) + (MICF_mean|F0/F0XXX)+(MICF_mean|F1/F1XXX)+
    (MICF_mean|F2/F2XXX)+(MICF_mean|F3/F3XXX)+(MICF_mean|F4/F4XXX)+(MICF_mean|F5/F5XXX)+
    (MICF_mean|F6/F6XXX)+(MICF_mean|F7/F7XX)+(MICF_mean|F8/F8XXX)+(MICF_mean|F9/F9XX),  data  = dfn, 
  prior = mod_priors,
  family=gaussian()
  #,  cores = 4
)

summary(mod_regression_rs)
plot(mod_regression_rs)

brms::prior_summary(mod_regression_rs)


pp<-predict(mod_regression_rs, re_formula =NULL,summary=F, robust=T)
boxplot(pp)

### check the model fit
pp_check(mod_regression_rs, ndraws=100)  
(r2<-bayes_R2(mod_regression_rs)) # Returns mean and CI for Bayesian R²)
# bayes_R2(mod_regression_rs) 
#      Estimate  Est.Error     Q2.5     Q97.5
#   R2 0.6322858 0.01140935 0.609126 0.6531825
loo(mod_regression_rs)    # Leave-one-out cross-validation
# Fitted (predicted) values
pred <- fitted(mod_regression_rs, summary = TRUE,robust=T, re_formula=NULL)  # returns mean and 95% CI
head(pred)
# Combine with observed data

results <- tibble(
  observed = mod_regression_rs$data$rWC,
...
p<-ggplot(results, aes(x = predicted, y = observed)) +
  ...
  theme_minimal()
print(p)
ggsave(filename = file.path("Figures/Bayesian","LMER_predictions_psy_FX_nested_FXXX_random_slopes.tiff" ),
...
get_variables(mod_regression_rs)
...
server <- function(input, output) {
  observeEvent(input$predict, {
    ...
    pp<-add_predicted_draws(newdata=newdata, mod_regression_rs) 
    head(pp)
