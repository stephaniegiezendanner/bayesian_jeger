...
# # Marginal R2 / Conditional R2	0.535 / 0.585

modelal = lmer(rWC ~ MICF_mean+Regeln.und.Routinen+ Aufgabenplanung+Flexibilitat+
                 ...
                 (MICF_mean|F8/F8XXX)+(MICF_mean|F9/F9XX) , #+ (1|F3) + (1|F3XX) + (1|F3XXX) 
               REML=T, data=dfn)
summary(modelal)
tab_model(modelal)
ranef(modelal)
...
mod_regression_d_rs = brms::brm(
  rWC ~ MICF_mean +
    ...
    (MICF_mean|F4/F4XXX)+(MICF_mean|F5/F5XXX)+
    (MICF_mean|F6/F6XXX)+(MICF_mean|F7/F7XX)+
    (MICF_mean|F8/F8XXX)+(MICF_mean|F9/F9XX),  data  = dfn, 
  prior = mod_priors,
  family=gaussian(), 
  chains = 4,
  iter =4000
...
summary(mod_regression_d_rs)
### check the model fit
pp_check(mod_regression_d_rs, ndraws=100)
plot(mod_regression_d_rs)
posterior_summary(mod_regression_d_rs) 
posterior_samples(mod_regression_d_rs) 
brms::prior_summary(mod_regression_d_rs)
pred <- as.data.frame(fitted(mod_regression_d_rs, robust=T, re_formula=NULL))  # returns mean and 95% CI
head(pred)
pred$obs<-mod_regression_d_rs$data$rWC
...
summary(lm(pred$obs~ pred$Estimate_adj))
#  0.6489 
  
(r2<-bayes_R2(mod_regression_d_rs, robust=T)) # Returns mean and CI for Bayesian R²)
# bayes_R2(mod_regression_d_rs) 
#      Estimate  Est.Error     Q2.5     Q97.5
# R2 0.6276148 0.01146909 0.6042692 0.6490554
loo(mod_regression_d_rs)    # Leave-one-out cross-validation
# Fitted (predicted) values
pred <- fitted(mod_regression_d_rs, summary = TRUE,robust=T, re_formula=NULL)  # returns mean and 95% CI
head(pred)
# Combine with observed data

results <- tibble(
  observed = mod_regression_d_rs$data$rWC,
...
get_variables(mod_regression_d_rs)
...
server <- function(input, output) {
  observeEvent(input$predict, {
    ...
    pp<-add_predicted_draws(newdata=newdata, mod_regression_d_rs) 
    head(pp)
